...
class Analyzer:
    def analyze(self, app: App): self.scope = Scope(); self.visit(app); return app
    def visit(self, node):
    ...
    def visit_app(self, app: App):
        self.validate_target(app.target); self.check_duplicate_pages(app); self.visit(app.system); self.visit(app.models); self.visit_auth(app.auth); self.visit_permissions(app.permissions, app); self.visit_routes(app.routes, {page.name for page in app.pages}, app.permissions)
        for page in app.pages: self.scope.define(Symbol(page.name, page))
        for page in app.pages: self.visit(page)
    def visit_system(self, system: System): self.visit(system.frontend); self.visit(system.backend); self.visit(system.database); self.visit(system.cache); self.visit(system.storage)
    ...
    def visit_auth(self, auth: Auth | None):
        ...
        if not providers and any(x is not None for x in (auth.registration, auth.login, auth.verification, auth.password_recovery, auth.mfa)): raise SemanticError("Authentication configuration requires at least one provider.")
        if auth.registration and auth.registration.verification and auth.registration.verification not in provider_set: raise SemanticError(f"Unknown registration verification provider '{auth.registration.verification}'.")
        if auth.login:
            for provider in auth.login.allow:
                if provider not in provider_set: raise SemanticError(f"Unknown login provider '{provider}'.")
        if auth.password_recovery and auth.password_recovery.method and auth.password_recovery.method not in provider_set: raise SemanticError(f"Unknown password recovery method '{auth.password_recovery.method}'.")
        if auth.verification and auth.verification.method and auth.verification.method not in provider_set: raise SemanticError(f"Unknown verification method '{auth.verification.method}'.")
        if auth.session and auth.session.timeout and not DURATION_RE.fullmatch(auth.session.timeout): raise SemanticError(f"Invalid session timeout '{auth.session.timeout}'.")
        if auth.mfa and auth.mfa.enabled and not auth.mfa.method: raise SemanticError("Enabled MFA must define a method.")
    def visit_routes(self, routes: Routes | None, page_names: set[str] | None = None, permissions: Permissions | None = None):
        ...
        for route in routes.routes:
            ...
            if route.auth and route.auth not in role_names: raise SemanticError(f"Route '{route.name}' references unknown auth role '{route.auth}'.")
    def visit_permissions(self, permissions: Permissions | None, app: App):
        ...
        permission_names = set(); resource_names = set()
        ...
        graph = {r.name: list(r.inherits) for r in permissions.roles}; visiting, visited = set(), set()
        for role in permissions.roles:
            if len(role.permissions) != len(set(role.permissions)): raise SemanticError(f"Conflicting duplicate permission references in role '{role.name}'.")
    ...
    def visit_page(self, page: Page):
        self.validate_theme(page.theme); self.check_duplicate_sections(page); self.visit(page.hero); page_scope = Scope(parent=self.scope); old = self.scope; self.scope = page_scope
