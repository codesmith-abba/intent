...
class ProjectService:
    ...
    def discover(self, path: str | Path) -> Path:
        try:
            ...
            raise CLIServiceError(
                f"Unable to find an ITL project from '{path}'. Run 'itl init <project>' to create one."
            ) from error

    def app_file(self, project: Path) -> Path:
        entrypoint = project / "app.itl"
        if not entrypoint.is_file():
            raise CLIServiceError(f"Project '{project}' has no app.itl entrypoint.")
        return entrypoint
    ...
    def check(self, path: str | Path):
        ...
        try:
            ...
            raise CLIServiceError(f"Check failed for '{project}': {error}") from error
    ...
    def explain(self, path: str | Path):
        ...
        try:
            ...
            raise CLIServiceError(f"Unable to explain '{project}': {error}") from error
    ...
    def build(self, path: str | Path, *, dry_run: bool = False):
        ...
        try:
            ...
            raise CLIServiceError(f"Build failed for '{project}': {error}") from error
        if dry_run:
            return {"project": project, "status": "dry-run", "cache_status": decision.status.value, "cache_reason": decision.reason, "entrypoint": entrypoint}
        output_root = paths.project
        try:
            IRWriter(output_root).write(ir)
        except Exception as error:
            raise CLIServiceError(f"Build output failed for '{project}': {error}") from error
        cache.put(str(entrypoint), output=str(output_root / "app.json"), metadata={"status": "success"})
        cache.save()
        return {"project": project, "status": "success", "cache_status": decision.status.value, "cache_reason": decision.reason, "entrypoint": entrypoint, "output": output_root / "app.json"}
        ...
        *,
        ...
        try:
            ...
            (browser_root / "index.html").write_text(
                ...
                "<script>new ITLBrowserRuntime.BrowserRuntime({root: document.getElementById('app')}).start('./runtime.json');</script>\n"
                "</body></html>\n",
                encoding="utf-8",
            )
        except Exception as error:
            raise CLIServiceError(f"Development build failed for '{project}': {error}") from error
        return project

    def graph(self, path: str | Path) -> str:
        project = self.discover(path)
        paths = ProjectPaths(project)
        candidates = (paths.graph / "graph.json", paths.project / "graph.json")
        ...
        if graph_file is not None:
            try:
                ...
                raise CLIServiceError(f"Unable to read project graph: {graph_file}") from error
    ...
    def plan(self, path: str | Path) -> dict[str, object]:
        ...
        return {"project": project, "entrypoint": entrypoint, "action": action, "cache_status": decision.status.value, "reason": decision.reason}
    ...
    def init(self, path: str | Path) -> Path:
        ...
        paths = ProjectInitializer().initialize(root, ProjectState(name=root.name, entrypoint="app.itl"))
