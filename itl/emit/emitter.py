        ...
        *,
        ...
        for item in plan.items:
            ...
            if not isinstance(result.output, str):
                output.add(EmitResult(item.source, EmitStatus.FAILED, error=EmitError("Emitter requires text output.")))
                continue

            try:
                path = self._safe_path(self._mapped_path(item.source))
                status = self._write(path, result.output, dry_run=dry_run)
                output.add(EmitResult(item.source, status, path=path))
                if not dry_run:
                    manifest[item.source] = path.relative_to(self.target_root).as_posix()
            except Exception as error:
                output.add(EmitResult(item.source, EmitStatus.FAILED, error=error))

        current_sources = {item.source for item in plan.items}
        for source in sorted(plan.removed):
            ...
            try:
                ...
                if path.exists():
                    if not dry_run:
                        path.unlink()
                    output.add(EmitResult(source, EmitStatus.REMOVED, path=path))
                else:
                    output.add(EmitResult(source, EmitStatus.UNCHANGED, path=path))
                if not dry_run:
                    manifest.pop(source, None)
            except Exception as error:
                output.add(EmitResult(source, EmitStatus.FAILED, error=error))

        stale = sorted(set(manifest) - current_sources - set(plan.removed))
        for source in stale:
            try:
        ...
        if not dry_run and not output.failed:
